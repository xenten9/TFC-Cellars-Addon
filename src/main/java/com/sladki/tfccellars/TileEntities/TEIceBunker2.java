...
public class TEIceBunker2 extends TEIceBunker {
    ...
    private void updateCellar(boolean checkCompliance) {
        if (avgYearTemp == Float.MIN_VALUE) {
            for (int month = 0; month < 12; month++) {
                avgYearTemp += TFC_Climate
                    .getHeightAdjustedTempSpecificDay(worldObj, month * TFC_Time.daysInMonth, xCoord, yCoord, zCoord);
            }
                ...
                if (lastUpdate < TFC_Time.getTotalDays()) {
                    if (outsideTemp > -10) { // magic
                        ...
                        coolantAmount = coolantAmount - (int) (ModConfig.coolantConsumptionMultiplier
                            * (0.05 * volume * (1 + lossMult) * (outsideTemp + volume + 2)));
                    }
                    lastUpdate++;
                }
            }
    ...
    private float doorsLossMult() {
        ...
        // 1st door
        Block door = worldObj.getBlock(xCoord + entrance[0], yCoord + 1, zCoord + entrance[1]);
        if (door == ModManager.CellarDoorBlock
            && ((BlockCellarDoor) door).isDoorOpen(worldObj, xCoord + entrance[0], yCoord + 1, zCoord + entrance[1])) {
        ...
        if (door == ModManager.CellarDoorBlock && ((BlockCellarDoor) door)
            .isDoorOpen(worldObj, xCoord + entrance[0] + entrance[2], yCoord + 1, zCoord + entrance[1] + entrance[3])) {
    ...
    private boolean isStructureComplete() {
            ...
            for (int x = -size[1] - 1; x <= size[3] + 1; x++) {
                for (int z = -size[2] - 1; z <= size[0] + 1; z++) {
                    ...
                    if (blockType == 1) {
                        ...
                        if (ModConfig.isDebugging) {
                            System.out
                                .println("Cellar at " + xCoord + " " + yCoord + " " + zCoord + " has too many doors");
                        }
                        error = 3;
                        return false;
                    }
        ...
        if (entrance[0] == 0 && entrance[1] == 0) {
            if (ModConfig.isDebugging) {
                System.out.println("Cellar at " + xCoord + " " + yCoord + " " + zCoord + " has no doors");
            }
            error = 3;
            return false;
        }
        ...
        for (int y = 0; y < 4; y++) {
            for (int x = -MathHelper.abs_int(entrance[3]); x <= MathHelper.abs_int(entrance[3]); x++) {
                for (int z = -MathHelper.abs_int(entrance[2]); z <= MathHelper.abs_int(entrance[2]); z++) {
                            ...
                            if (ModConfig.isDebugging) {
                                System.out.println(
                                    "Cellar at " + xCoord
                                        ...
                                        + " "
                                        + zCoord
                                        + " doesn't has the second door, block there is "
                                        + blockType);
                            }
                        }
                    }
                    ...
                    if (ModConfig.isDebugging) {
                        System.out.println(
                            "Doors at the cellar at " + xCoord
                                + " "
                                + yCoord
                                + " "
                                + zCoord
                                + " doesn't surrounded by wall, block there is "
                                + blockType);
                    }
                }
            }
    ...
    private int getBlockType(int x, int y, int z) {
        ...
        if (block == ModManager.CellarWallBlock) {
            ...
            return 1;
        } else if (block == ModManager.CellarShelfBlock || block == TFCBlocks.barrel
